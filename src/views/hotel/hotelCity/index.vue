...
export default {
  ...
  methods: {
    ...
    searcharea(cityid) {
      if (cityid != null) {
        selectCityAreaByCityId({ cityId: cityid }).then(res => {
          console.log(res);
    ...
    handleClose1(tag) {
      ...
      console.log(this.searchinfo.dynamicTags);
      if (tag.hotelId == 0) {
        this.searchinfo.dynamicTags.splice(
          this.searchinfo.dynamicTags.findIndex(v => {
            return v.areaName == tag.areaName;
          }),
          1
        );
        if (tag.id != 0) {
          ...
          this.searchinfo.addinfo.splice(
            this.searchinfo.addinfo.findIndex(v => {
              return v.areaName == tag.areaName;
            }),
            1
          );
    ...
    handleInputConfirm1() {
      ...
      if (this.inputValue1) {
        if (
          this.searchinfo.dynamicTags.findIndex(v => {
            return v.areaName == this.inputValue1;
          }) != -1
        ) {
    ...
    areasure() {
      ...
      // console.log(this.searchinfo.addinfo);
      var addarea = {
        areas: this.searchinfo.addinfo,
        cityId: this.searchform.city
      };
      var deletearea = {
        areaIds: this.searchinfo.deleteid,
        cityId: this.searchform.city
      };
      deleteAreaByAreaId(deletearea).then(res => {
        if (res.message == "SUCCESS") {
          insertAreas(addarea).then(res => {
            if (res.message == "SUCCESS") {
