    ...
    <div class="HotelList-main-container-search">
      <el-form :inline="true" :model="searchform" class="container-search-form">
        <el-form-item label="城市区域">
          <el-select v-model="searchform.city_id" filterable placeholder="请选择" clearable>
            <el-option
        ...
        <el-form-item>
          <el-button type="success" style="width:100px" @click="tosearch">查询酒店</el-button>
        </el-form-item>
        <el-form-item>
            ...
            <el-popover trigger="hover" placement="top">
              <div>
                <el-button
                  ...
                  >在售</el-button
                >
                <el-button type="danger" size="mini" @click="uploadhotel(scope.row,1)"
                  >停售</el-button
                >
              </div>
      ...
      <el-dialog :visible.sync="dialogFormVisible" :title="title">
        ...
        >
          ...
          <el-form-item label="酒店介绍">
            <el-input v-model="changeinfoall.introduce" type="textarea" rows="3"></el-input>
          </el-form-item>
          <el-form-item label="联系方式" >
            <el-input style="width:180px;" v-model="changeinfoall.telephone" type="number"></el-input>
          </el-form-item>
          ...
          <el-form-item label="酒店图集" v-if="!addhotel">
            <template >
              <div class="flexrow imgflex" >
                <img v-if="urls.length==0" style="width: 280px; height: 180px" src="@img/noimg.png" alt="" />
                <!-- <el-image
                ...
                <div class="hotelimgadmin">
                  <el-button size="small" type="primary" @click="hotelimgVisible=!hotelimgVisible">管理</el-button>
                </div>
              </div>
            </template>
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <!-- 新增酒店 -->
          <el-button type="primary" @click="addapproval('changeinfoall')" v-if="!changehotel">确 定</el-button> 
          <!-- 修改酒店 -->
          <el-button type="primary" @click="updataapproval('changeinfoall')" v-if="!addhotel">确 定</el-button>
        </div>
      </el-dialog>
      <!-- 图集管理 -->
      <el-dialog :visible.sync="hotelimgVisible">
        <div class="img-show-view">
          <div class="img-show-item" v-for="url in urls" :key="url.id">
            ...
            <span class="delete-icon" @click="deleteimg(url.id)"><i class="el-icon-error"></i></span>
            <el-image
              style="width: 210px; height: 100px"
              :src="url.url"
              fit="fit">
                <div slot="placeholder" style="width: 210px; height: 100px;text-align:center;line-height: 100px">
...
<script>
import Qs from "qs";
import { insertHotel,insertHotelArea,selectAlbums,selectBrands,selectAllCities,selectHotelapi,updateByPrimaryKey,selectHotelareaapi,deleteAlbumapi,selectAllCityAreas} from "@/api/api.js";
...
export default {
  ...
  watch: {
    ...
    changecity(newName, oldName) {
      ...
      if(newName!=null){
        ...
        this.childMenu=this.CityAreaList.find(item=>item.id==newName).areas;
  ...
  methods:{
      ...
      selectHotelapi(searchparm)
        .then(res=>{
          ...
          for(var i=0;i<this.tableData.length;i++){
            ...
            this.tableData[i].lastConfirm=this.tableData[i].lastConfirm==null?'00:00:00':this.tableData[i].lastConfirm;
            this.tableData[i].lastOrder=this.tableData[i].lastOrder==null?'00:00:00':this.tableData[i].lastOrder;
          }
    ...
    suretime() {
      ...
      var changeparm={id:this.changetime.id+'',lastConfirm:this.changetime.lastConfirm,lastOrder:this.changetime.lastOrder};
    ...
    async updataapproval(changeinfoall){
      ...
      if(this.checkedEquipments.length!=0){
        var newhotelinfo={ hotelName:'',brandId:null,saleStatus:null,lastOrder:null,lastConfirm:null,star:null,address:null,telephone:null,introduce:null,remain:null,id :null};
        ...
        let updateKey= await updateByPrimaryKey(newhotelinfo);
         if(updateKey.message=="SUCCESS"){
           let issuccess=null
           if(!(this.changeinfoall.city.id==this.changecity&&this.checkedEquipments==this.saveEquipments)){
             //更改了城市区域
            // console.log("更改了城市区域");
            var newarea=[...new Set(this.checkedEquipments)]
            issuccess= await insertHotelArea({hotel_id:this.changeinfoall.id,area_ids:newarea});
           }
    ...
    addapproval(changeinfoall) {
      this.$refs[changeinfoall].validate((valid) => {
          if (valid==true&&this.checkedEquipments.length!=0&&this.changecity!=null) {
            // console.log("新增酒店");
            // console.log(this.changeinfoall);
            var newhotelinfo={ hotelName:'',brandId:null,saleStatus:0,lastOrder:null,lastConfirm:null,star:null,address:null,telephone:null,introduce:null,remain:null,id :null};
            listAssign(newhotelinfo,this.changeinfoall)
